...
export const movie = (state = initialState, action) => {
  ...
  switch (type) {
	case GET_MOVIES_BY_SEARCH_QUERY:
		return {...state, searchQuery: payload.search, sortBy: payload.sortBy, isFetching: true}
	...
	case SET_SEARCH_FILTER:
		return {...state, searchBy: payload.searchBy}
	case SET_SORT_FILTER:
		return {...state, sortBy: payload.sortBy}
	case GET_MOVIE_DETAILS_BY_ID:
		return {...state, id: payload.id, isFetching: true}
	case GET_MOVIE_DETAILS_BY_ID_SUCCESS:
		return {...state, movie: payload.movie, isFetching: false}
